    ...
    <div class="col-md-12">
        <div class="btn-group btn-group-toggle btn-group-status" data-toggle="buttons">
            <label for="status_active" class="btn btn-outline-primary active">
                <input type="radio" name="filter_status" id="status_active" value="0">
                Menunggu persetujuan ({{ $count_pending }})
            </label>
            <label for="status_inactive" class="btn btn-outline-primary">
                <input type="radio" name="filter_status" id="status_inactive" value="1">
                Disetujui ({{ $count_approve }})
            </label>
            <label for="status_inactive" class="btn btn-outline-primary">
                <input type="radio" name="filter_status" id="status_inactive" value="2">
                Ditolak ({{ $count_disapprove }})
            </label>
        </div>
    </div>
</div>
    ...
    <div class="col-12 mt-4 pt-1">
        <div class="border-1">
            ...
            <div class="table-responsive">
                <table class="table delete-company-account" style="width:100%">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        @foreach ($deleted as $data)
                            <tr>
                                ...
                                <td> {{ substr($data->reason, 0, 50) }} </td>
                                <td> {{ $data->company_name }} </td>
                                <td class="text-center {{ $data->is_active == 0 ? "text-warning" : ($data->is_active == 1 ? "text-success" : "text-danger") }}">
                                    <i class="mdi mdi-check-circle"></i>
                                    {{ $data->is_active == 0 ? "Pending" : ($data->is_active == 1 ? "Disetujui" : "Ditolak") }}
                                </td>
...
<script>
    $(() =>
    {
        var table = $('.delete-company-account').DataTable({
            lengthChange: false,
            destroy: true,
            columnDefs: [{
                orderable: false,
        ...
        $('input[name=filter_status]').on('change', function()
        {
            ...
            let val = status[this.value];
            table.column(6).search(val, true, false, false).draw();
        });

        table.column(6).search('Pending', true, false, false).draw();
    })
</script>
@endpush
