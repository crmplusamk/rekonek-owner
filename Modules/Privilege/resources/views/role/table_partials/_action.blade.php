...
<div class="dropdown-menu dropdown-menu-right" role="menu" x-placement="bottom-end" style="position: absolute; transform: translate3d(-144px, 18px, 0px); top: 0px; left: 0px; will-change: transform;">
    ...
    <a class="dropdown-item pointer" data-toggle="modal" data-target="#status-{{ $role->id }}">
        <i class="mdi mdi-{{ $role->is_active ? "close" : "check" }}"></i> {{ $role->is_active ? "Nonaktifkan" : "Aktifkan" }}
    </a>
    <a class="dropdown-item pointer" data-toggle="modal" data-target="#destroy-{{ $role->id }}">
        <i class="mdi mdi-delete"></i>
        Hapus
    </a>
</div>

<div class="modal fade" id="status-{{ $role->id }}" aria-labelledby="status" aria-hidden="true">
        ...
        <div class="modal-content">
            <div class="modal-body modal-body-lg text-center text-wrap text-break">
                ...
                <div class="text-center mt-4">
                    <h5>Konfirmasi {{ $role->is_active ? "Nonaktifkan" : "Aktifkan" }} Data</h5>
                    <p class="mt-2 text-lg">Apakah anda yakin ingin {{ $role->is_active ? "menonaktifkan" : "mengaktifkan" }} role {{ $role->alias }} ? </p>
                ...
                <div class="modal-action mt-4 mb-3 d-flex justify-content-center align-items-center">
                    ...
                    <a href="{{ route('role.status', $role->id) }}" class="btn btn-primary mr-3">
...
<div class="modal fade" id="destroy-{{ $role->id }}" aria-labelledby="destroy" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-body modal-body-lg text-center text-wrap text-break">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                ...
                <div class="text-center mt-4">
                    ...
                    <p class="mt-2 text-lg">Apakah anda yakin ingin menghapus role {{ $role->alias }} ? </p>
                    <p class="text-danger">Proses ini tidak dapat dibatalkan</p>
                </div>
                <div class="modal-action mt-4 mb-3 d-flex justify-content-center align-items-center">
                    <a data-dismiss="modal" class="btn btn-danger mr-3">
