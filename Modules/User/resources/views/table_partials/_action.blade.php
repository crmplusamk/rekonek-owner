...
<div class="dropdown-menu dropdown-menu-right" role="menu" x-placement="bottom-end" style="position: absolute; transform: translate3d(-144px, 18px, 0px); top: 0px; left: 0px; will-change: transform;">
    ...
    <a class="dropdown-item pointer" data-toggle="modal" data-target="#status-{{ $user->id }}">
        <i class="mdi mdi-{{ $user->is_active ? "close" : "check" }}"></i> {{ $user->is_active ? "Nonaktifkan" : "Aktifkan" }}
    </a>
...
<div class="modal fade" id="status-{{ $user->id }}" aria-labelledby="status" aria-hidden="true">
        ...
        <div class="modal-content">
            <div class="modal-body modal-body-lg text-center text-wrap text-break">
                ...
                <div class="text-center mt-4">
                    <h5>Konfirmasi {{ $user->is_active ? "Nonaktifkan" : "Aktifkan" }} Data</h5>
                    <p class="mt-2 text-lg">Apakah anda yakin ingin {{ $user->is_active ? "menonaktifkan" : "mengaktifkan" }} user {{ $user->alias }} ? </p>
                ...
                <div class="modal-action mt-4 mb-3 d-flex justify-content-center align-items-center">
                    ...
                    <a href="{{ route('user.status', $user->id) }}" class="btn btn-primary mr-3">
...
<div class="modal fade" id="destroy-{{ $user->id }}" aria-labelledby="destroy" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-body modal-body-lg text-center text-wrap text-break">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                ...
                <div class="text-center mt-4">
                    ...
                    <p class="mt-2 text-lg">Apakah anda yakin ingin menghapus user {{ $user->name }} ? </p>
                    <p class="text-danger">Proses ini tidak dapat dibatalkan</p>
                </div>
                <div class="modal-action mt-4 mb-3 d-flex justify-content-center align-items-center">
                    <a data-dismiss="modal" class="btn btn-danger mr-3">
